...
function shouldReplyInThread({
    ...
    if (opId && authorId && authorId !== opId) {
        // ...with one exception: they handed us a FILE. A second person hitting
        // the same bug uploads their own latest.log; a helper answering the OP
        // argues in prose. Blanket-ignoring every non-author meant a log posted
        // in reply to "can you send your log?" was silently dropped, and the
        // person who bothered to upload it got nothing back.
        //
        // Both conditions keep this narrow: Shubba spoke last (so the upload is
        // plainly a response to us), or no other human is active (so there is no
        // side conversation to intrude on).
        if (hasAttachments && (lastResponderWasShubba || others.length === 0)) {
            return { reply: true, reason: 'non-author supplied a file' };
        }
