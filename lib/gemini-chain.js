...
function createGeminiRunner(cfg) {
     ...
     * @param {function} [opts.genConfigFor] (model) => generationConfig. Overrides
     *        `genConfig` per model. Needed because generationConfig is NOT
     *        model-agnostic: a knob the front model accepts (e.g. thinkingConfig)
     *        can be rejected by a model further down the chain, and that comes
     *        back as a 400 — which aborts the whole chain by design. Without
     *        this, the cheap tail model can never actually serve as the safety
     *        net for requests configured for the smart front model.
    ...
    async function runGeminiChain(prompt, opts) {
        const { models, genConfig, genConfigFor, timeout = 120000, label = 'gemini', perModelRetries = 1 } = opts;
        const configFor = typeof genConfigFor === 'function' ? genConfigFor : () => genConfig;
            ...
            while (true) {
                try {
                    ...
                    const res = await httpPost(
                        buildUrl(model),
                        { contents: [{ parts: [{ text: prompt }] }], generationConfig: configFor(model) },
