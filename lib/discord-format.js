...
function stripMentions(text) {
    return String(text ?? '')
        .replace(/<@!?(\d+)>/g, 'someone')
        .replace(/<@&(\d+)>/g, 'the team')
        .replace(/@everyone/g, 'everyone')
        .replace(/@here/g, 'here')
        // Bare "@kewz." does not technically ping, but it READS as a summons —
        // a real user apologised for pinging the devs over text Shubba had
        // written itself. Drop the leading @ from anything shaped like a handle.
        //
        // The (^|[\s(\[]) guard is what preserves email addresses: in
        // "a@b.com" the @ follows a word character, so it is not a handle and
        // is left alone.
        .replace(/(^|[\s(\[])@([a-z0-9._-]{2,32})/gi, '$1$2');
}
